...
class Club:
    def __init__(self, nombre, descripcion, ubicacion, presidente, fecha_fundacion):
        if not isinstance(fecha_fundacion, date):
            raise TypeError("fecha_fundacion debe ser un objeto date, no un str") #Segun lo entendido. El raise TypeError sirve para detener la ejecucion, para indicar el error
        self.nombre = nombre
...
if __name__ == "__main__": #Acá lo que quiere decir es estas líneas de prueba corrélas solo cuando yo abra este archivo, no cuando otro archivo lo use de préstamo
